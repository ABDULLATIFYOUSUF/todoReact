    ...
    <div className='bg-gray-200 w-full h-screen flex items-center'>
      <div className='w-[500px] mx-auto bg-white p-5 text-center'>
        ...
        <form onSubmit={handleForm}>
          <input className='border-2 border-black w-full p-5 text-black placeholder:text-gray-500 rounded-lg mb-5' type="text" placeholder='Add Todo' value={todo} onChange={(e) => setTodo(e.target.value)} />
        ...
        <div className="todo-show-area">
          <ul>
            {todoList.map((singleTodo, index)=>{
              return(
              <li key={index} className='bg-black mb-5 text-white py-5 rounded-lg px-5 text-3xl flex justify-between'>{singleTodo.todoName}{""} <span onClick={() => deleteTodo(singleTodo.todoName)} className='text-red-600 cursor-pointer'>x</span> </li>
              )
            })}
